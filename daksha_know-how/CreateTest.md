 ...
 * **task**- This will contain the steps of your test.For writing this section please take a look at the list of functionalities that we are providing in the below list.You can perform simple to complex test cases with a combination of the below functionalities.
   ...
   * **download_file**: This has two sub-fields; *locator* and *save_in*. **Locator** is the file path that needs to be downloaded. It can be *xpath, id, css, name, tagname, classname, linktext and partiallinktext*. **Save_in** is optional in case user wants to read the downloaded file.
   * **read_file**: This is to read a downloaded file in the same session. Pass the *save_in* key previously provided in **download_file**
